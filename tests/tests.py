...
class test_sweeper(unittest.TestCase):
    def make_block_bytes(self, size: int):
        ...
        for _ in range(0, (2 * size) // len(hex_chars)):
            ...
            block_bytes.extend(bytearray.fromhex(
                ''.join(hex_chars[(2 * start):])))
            block_bytes.extend(bytearray.fromhex(
                ''.join(hex_chars[0:(2 * start)])))

        tail = size - len(block_bytes)
        if tail > 0:
            random.shuffle(hex_chars)
            block_bytes.extend(bytearray.fromhex(
                ''.join(hex_chars[0:(2 * tail)])))
    ...
    def build_file_name(self, serial: int, prefix: str, blocks: int, tail: int, size: int) -> str:
    ...
    def generate_file_data(self):
        ...
        for k in range(0, 2):
            ...
            for i in range(0, max_files):
                tail = random.randint(1, hash_config.block_size() - 1)
            ...
            for i in range(0, 2):
                ...
                if not flag_repo:
                    for j in range(0, random.randint(0, 4)):
                        dir_clone = dir_redu if 0 == random.randint(
                            0, 1) else dir_dest
                        file_path = os.path.join(
                            dir_clone, f"{name}.clone.{j + 1}")
                        ...
                        if 1 == random.randint(1, 3):
                            file_path = os.path.join(
                                dir_clone,  f"{name}.fake")
                            fake_block_bytes = self.make_block_bytes(
                                len(block_bytes))
                            with open(file_path, 'wb') as file:
